  ...
  try {
    ...
    if (props.mode === 'add') {
      const result = await window.llmApi.addConfig({
        ...
        embeddingModel: formData.value.embeddingModel.trim() || undefined,
          ...
          <template v-else-if="configType === 'local'">
            ...
            <div>
               <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                ...
                <UInput v-model="formData.embeddingModel" :placeholder="t('embeddingModelNamePlaceholderLocal')" class="w-full" />
                <p class="mt-1 text-xs text-gray-500">{{ t('embeddingModelNameHint') }}</p>
            </div>
...
<i18n>
{
  "zh-CN": {
    ...
    "cancel": "取消",
    "add": "添加",
    "embeddingModelName": "Embedding 模型名称",
    "embeddingModelNamePlaceholder": "如 qwen3-embedding:4b、nomic-embed-text",
    "embeddingModelNamePlaceholderLocal": "如 qwen3-embedding:4b、nomic-embed-text",
    "embeddingModelNameHint": "用于语义检索 / RAG（可选，不填则不启用）",
    "save": "保存"
  },
  "en-US": {
    ...
    "embeddingModelNamePlaceholder": "e.g. qwen3-embedding:4b, nomic-embed-text",
    "embeddingModelNamePlaceholderLocal": "e.g. qwen3-embedding:4b, nomic-embed-text",
    "embeddingModelNameHint": "For semantic search / RAG (optional). Leave empty to disable.",
    "save": "Save"
  }
}
</i18n>
