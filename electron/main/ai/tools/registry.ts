...
async function embedTextsOpenAICompatible(input: string[]): Promise<number[][]> {
  aiLogger.info('Embedding', 'embedTextsOpenAICompatible called', {
    inputCount: input.length,
    preview: input.slice(0, 2),
  })
  const cfg = llm.getActiveConfig()
  if (!cfg) throw new Error('No active AI config')
  if (!cfg.baseUrl) throw new Error('Active config missing baseUrl')
  if (!cfg.embeddingModel) throw new Error('Active config missing embeddingModel')

  const url = cfg.baseUrl.replace(/\/$/, '') + '/embeddings'
  const apiKey = cfg.apiKey || 'sk-no-key-required'

  aiLogger.info('Embedding', 'embedding request', {
    url,
    model: cfg.embeddingModel,
    inputCount: input.length,
  })
  aiLogger.info('Embedding', 'POST /embeddings', { url, model: cfg.embeddingModel })
  ...
  if (!resp.ok) {
    throw new Error(`Embeddings request failed: ${resp.status} ${await resp.text()}`)
  }

  const data = await resp.json()

  aiLogger.info('Embedding', 'embedding response', {
    vectors: Array.isArray(data?.data) ? data.data.length : 0,
    dim: Array.isArray(data?.data) && data.data[0]?.embedding ? data.data[0].embedding.length : undefined,
  })
  
  return (data.data || []).map((x: any) => x.embedding as number[])
}
...
function topKByCosine(q: number[], vecs: number[][], k: number): { idx: number; score: number }[] {
  const scores = vecs.map((v, i) => ({ idx: i, score: cosine(q, v) }))
...
const semanticSearchMessagesTool: ToolDefinition = {
  ...
  function: {
    ...
    description:
      '用 embedding 语义相似度在聊天记录中检索相关消息。适用于关系/相处方式分析、抽象提问、找“类似表达”。需要在本地服务配置中填写 Embedding 模型名称。',
    parameters: {
      type: 'object',
      properties: {
        query: { type: 'string', description: '检索查询（必填）' },
        top_k: { type: 'number', description: '返回条数，默认 20（建议 10-30）' },
        candidate_limit: {
          type: 'number',
          description: '候选消息数量（从最近消息中取），默认 200（上限受“发送条数限制”约束）',
        },
        year: { type: 'number' },
        month: { type: 'number' },
        day: { type: 'number' },
...
async function semanticSearchMessagesExecutor(
  ...
  const { sessionId, timeFilter: contextTimeFilter, maxMessagesLimit, locale } = context
  aiLogger.info('SemanticSearch', 'executor called', {
    query: params.query,
    top_k: params.top_k,
  ...
  const topK = Math.max(1, Math.min(50, params.top_k ?? 20))

  const requestedCand = params.candidate_limit ?? 200
  const candidateLimit = maxMessagesLimit ? Math.min(requestedCand, maxMessagesLimit) : requestedCand

  const effectiveTimeFilter = parseExtendedTimeParams(params, contextTimeFilter)

  const cand = await workerManager.getRecentMessages(sessionId, effectiveTimeFilter, candidateLimit)
  const candMessages = cand.messages || []

  if (candMessages.length === 0) {
    return {
      query: params.query,
      top_k: topK,
  ...
  // 语义检索的输入不要太长，不然 embedding 会慢
  const texts = candMessages.map((m) => formatMessageCompact(m, locale).slice(0, 400))


  aiLogger.info('SemanticSearch', 'calling embedTextsOpenAICompatible', {
    query: params.query,
    textsCount: texts.length,
    topK,
    candidateLimit,
  })
  // 计算向量：query + candidates
  const vecs = await embedTextsOpenAICompatible([params.query, ...texts])
  aiLogger.info('SemanticSearch', 'embedding returned', {
    vectors: vecs.length,
    dim: Array.isArray(vecs?.[0]) ? vecs[0].length : undefined,
  })
  const qv = vecs[0]
  const dvs = vecs.slice(1)

  const top = topKByCosine(qv, dvs, Math.min(topK, candMessages.length))
  ...
  return {
    ...
    candidate_limit: candidateLimit,
    timeRange: formatTimeRange(effectiveTimeFilter, locale),
    results: top.map(({ idx, score }) => ({
      score,
      message: formatMessageCompact(candMessages[idx], locale),
    })),
  }
}
/**
